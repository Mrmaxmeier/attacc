  ...
  implicit def metricsEncoder[F[_]]: EntityEncoder[F, Metrics] = EntityEncoder.stringEncoder(Charset.`UTF-8`).contramap[Metrics] { value =>
    value
      ...
      .map { metric =>
        ...
        s"attacc_${metric.name}${if (labels.length > 0) "{" else ""}$labels${if (labels.length > 0) "}" else ""} ${metric.value}${metric.timestamp.map(l => s" $l").getOrElse("")}"
      }
      .mkString("\n")
  }

  def collect(state: ApplicationState): ZIO[ApplicationEnv, Throwable, Metrics] = {
    def sessionKey(session: Session) = session.hostname + ':' + session.path.replaceAll("\"", "").split("/").last
    for {
      ...
      flags.map { case (id, l)     => Metric("flags_accepted", l.sum.toString, None, "session"  -> id) }
        ++ runs.map { case (id, l) => Metric("runs_successful", l.sum.toString, None, "session" -> id) }
    )
  }
}
